...
- Runbook içeriği Wave 1 deploy kuyruğunda AWAITING operator action
  olarak korunuyor. vm3'te `audit-worm` ve `evidence-worm` bucket'ları
  henüz COMPLIANCE moduna alınmamış.
- SOC 2 Type II observation window'un başlayabilmesi için bu runbook
  canlı koşulmalı. Phase 3.0 evidence locker kod olarak hazır ama
  WORM anchor gerçek değil → evidence `dual-write` path şu an sadece
  soft-delete mirror'a yazıyor.
- `mc retention set --default compliance 1826d` komutu GERİ ALINAMAZ —
  bir kez uygulandıktan sonra 5 yıl boyunca bucket WORM'dur.
- İlgili: `secret-rotation.md` MinIO root rotation burası etkinleştikten
  sonra koşulmalı (rotation öncesi mevcut root'un audit kaydı çıkarılmalı).
