...
_RULES: list[tuple[str, str, float]] = [
    ...
    (r"\bparaşüt\b|\bparasut\b", "work", 0.98),
    (r"\bbordroplus\b", "work", 0.99),
    (r"\be-fatura\b", "work", 0.97),
    (r"\be-arşiv\b", "work", 0.97),
    (r"\beba\b.*\b(portal|login)\b", "work", 0.95),
    # Faz 8 #82 — additional Turkish business software
    (r"\bhitit\b.*(erp|muhasebe|yazılım)", "work", 0.96),
    (r"\beta\s+sql\b|eta\s+yazılım", "work", 0.97),
    (r"\bzirve\s+yazılım\b|zirve\s+(müşavir|bordro|muhasebe|defter)", "work", 0.96),
    (r"\bvega\s+yazılım\b|vega\s+(erp|muhasebe)", "work", 0.95),
    (r"\bsap\s+(gui|logon|business\s+one|b1|fiori|hana|crm)\b", "work", 0.98),
    (r"\bsaplogon\.exe\b|\bsapgui\.exe\b", "work", 0.98),
    (r"\boracle\s+(erp|fusion|netsuite|hyperion|primavera)\b", "work", 0.96),
    (r"\bnetsuite\b", "work", 0.96),
    (r"\bsage\s+(50|100|300|x3|accounting)\b", "work", 0.95),
    ...
    (r"\bjira\b|jira\.atlassian\.com|atlassian\.net", "work", 0.95),
    (r"\bconfluence\b|confluence\.atlassian\.com", "work", 0.95),
    (r"\bnotion\b|notion\.so", "work", 0.88),
    (r"\bairtable\b|airtable\.com", "work", 0.90),
    (r"\bfigma\b", "work", 0.93),
    (r"\blinear\b|linear\.app", "work", 0.90),
    (r"\basana\b|asana\.com", "work", 0.92),
    (r"\btrello\b|trello\.com", "work", 0.90),
    (r"\bmonday\.com\b|\bmonday\s+work\s+management\b", "work", 0.92),
    (r"\bclickup\b|clickup\.com", "work", 0.92),
    (r"\bbasecamp\b|basecamp\.com", "work", 0.90),
    ...
    (r"\bdisney\+?\b|disneyplus\.com|disney\.plus", "distraction", 0.97),
    ...
    # Spotify: often background music during real work; treat as neutral → unknown
    # so it doesn't artificially reduce productivity scores.
    (r"\bspotify\b", "unknown", 0.75),
    # Turkish distraction / forum sites
    (r"ek[sş]i\s*s[oö]zl[uü]k|eksisozluk\.com", "distraction", 0.92),
    (r"\binci\s*s[oö]zl[uü]k\b|incisozluk\.com", "distraction", 0.92),
    (r"\bdonan[iı]mhaber\b|donanimhaber\.com", "distraction", 0.88),
